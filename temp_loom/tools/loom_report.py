"""The full loom report -> a .txt to READ: traced stories + the questions the tape raised.

Composes the whole stack — tape -> events -> threads -> questions -> investigations -> text.
No player, no model, deterministic, milliseconds.

    python -m tools.loom_report --tape data/f1_barcelona_2026.json \
        --grammar data/grammars/intern.json --verbs data/english/irregular_verbs.json \
        --rules data/f1_causal_rules.json --inquiry data/inquiry/f1.json \
        --depth 2 --curiosity 3 --out transcripts/f1_report.txt
"""
...
from oradio_engine.inquiry import Inquiry, investigate
...
def _event(row: dict) -> dict:
    ...
    roles["lap"] = next((t.split(":", 1)[1] for t in row.get("tags", []) if t.startswith("lap:")), "")
...
def main() -> None:
    ...
    ap.add_argument("--inquiry", default="")
    ap.add_argument("--depth", type=int, default=2)
    ap.add_argument("--curiosity", type=int, default=3)
    ...
    questions = []
    if args.inquiry:
        qs = Inquiry.from_file(args.inquiry).ask(events, curiosity=args.curiosity)
        questions = investigate(events, grammar, qs, depth=args.depth, rules=rules)

    os.makedirs(os.path.dirname(args.out) or ".", exist_ok=True)
    with open(args.out, "w", encoding="utf-8") as f:
        f.write("=" * 60 + "\nTRACED STORIES (threads, depth %d)\n" % args.depth + "=" * 60 + "\n\n")
        for lap, line in stories:
            f.write((f"[lap {lap}] " if lap else "") + line + "\n")
        f.write("\n" + "=" * 60 + "\nQUESTIONS THE TAPE RAISED (curiosity %d)\n" % args.curiosity + "=" * 60 + "\n\n")
        for q, thread in questions:
            f.write("?  " + q + "\n   -> " + thread + "\n\n")

    elapsed = time.perf_counter() - t0
    print(f"{len(events)} events -> {len(stories)} stories, {len(questions)} questions")
