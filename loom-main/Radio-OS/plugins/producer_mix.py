...
PLUGIN_NAME = "producer_mix"

IS_FEED = False

# =====================================================
# Mix Rebalance Widget (Producer Queue Focused)
# =====================================================
...
def register_widgets(registry, runtime):
    ...
    def factory(parent, runtime):
        ...
        header = tk.Label(
            card,
            text="Producer Feed Queue Rebalance",
            bg=CARD,
            fg=TEXT,
        ...
        subtitle = tk.Label(
            card,
            text="Adjust the feed queue priorities based on producer's logic",
            bg=CARD,
            fg=MUTED,
            font=("Segoe UI", 10),
            anchor="w"
        ...
        # =====================================================
        # Data helpers (Producer's Queued Feeds)
        # =====================================================

        def get_feeds() -> list[str]:
            cfg = get_cfg()

            # Modify to get producer-specific feed data
            feeds = cfg.get("feeds")
            ...
            if isinstance(quotas, dict) and quotas:
                return [str(k) for k in quotas.keys()]
    ...
    registry.register(
        "mix_rebalance",
        factory,
        title="Producer Feed Queue Rebalance",
        default_panel="right"
    )
