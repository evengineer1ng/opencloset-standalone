...
class RadioShell:
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title(f"Radio OS {RADIO_OS_VERSION}")
    ...
    def _build_top_bar(self):
        ...
        self.title_lbl = tk.Label(bar, text=f"Radio OS {RADIO_OS_VERSION}", font=FONT_H1, fg=UI["text"], bg=UI["bg"])
    ...
    def _create_station_card(self, parent, station: StationInfo, idx: int) -> tk.Frame:
        ...
        # Pack buttons FIRST when using side="bottom" (tkinter geometry requirement)
        actions = tk.Frame(card, bg=UI["card"])
        actions.pack(fill="x", padx=int(14 * UI_SCALE), pady=(0, int(16 * UI_SCALE)), side="bottom")

        # Feeds/info area - fixed height
        feeds_frame = tk.Frame(card, bg=UI["card"])
        feeds_frame.pack(fill="x", padx=14, pady=(0, int(12 * UI_SCALE)))
        ...
        # Add flexible spacer between content and buttons
        tk.Frame(card, bg=UI["card"]).pack(fill="both", expand=True)
