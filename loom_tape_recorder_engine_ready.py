...
- .tape.json    booth upload-loader compatible tagged rows with string raw (prevents [object Object])
...
def tag_value_from_row(row: Dict[str, Any], key: str) -> str:
    prefix = f"{key}:"
...
def classify_topic(text: str, source: str = "") -> str:
    lower = f"{text} {source}".lower()
    buckets = [
        ("ai_models", ("llm", "model", "open-weight", "open weights", "weights", "qwen", "glm", "mistral", "claude", "gpt", "ollama", "hugging", "benchmark", "terminal-bench")),
        ("markets", ("stock", "market", "wall street", "surge", "bitcoin", "btc", "price", "finance", "%", "earnings")),
        ("sports", ("f1", "formula", "race", "lap", "goal", "game", "championship", "playoff", "nba", "nhl")),
        ("weather", ("weather", "storm", "rain", "snow", "temperature", "wind", "forecast")),
        ("software", ("github", "release", "api", "code", "coding", "dataset", "open source", "opensource", "bug", "deploy")),
        ("incident", ("error", "failed", "down", "outage", "alert", "warning", "crash", "fire")),
        ("news", ("breaking", "report", "says", "announces", "launches", "article")),
    ]
    for topic, words in buckets:
...
def extract_entities(text: str, limit: int = 8) -> List[str]:
    # Deterministic, cheap entity hints. No model. Good enough for thread keys.
    candidates: List[str] = []
    patterns = [
        r"\b[A-Z][A-Za-z0-9]*(?:[-_.][A-Za-z0-9]+)+\b",       # GLM-5.2, Terminal-Bench
        r"\b[A-Z]{2,}(?:\d+(?:\.\d+)?)?\b",                  # API, BTC, QWEN2.5
        r"\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,3}\b",           # Proper Names
        r"\b\d+(?:\.\d+)?%\b",                               # 80%, 33%
    ]
    for pat in patterns:
        for m in re.finditer(pat, text):
            ent = clean_text(m.group(0), 80)
            if ent and ent.lower() not in {"the", "this", "that", "and", "with", "from"} and ent not in candidates:
...
def infer_intensity(text: str, valence: str, priority: float) -> float:
    lower = text.lower()
    score = 0.25 + 0.55 * clamp(priority)
    if valence == "hype":
        score += 0.12
    if valence == "alarm":
        score += 0.18
    if "!" in text:
        score += 0.06
    if any(w in lower for w in ("breaking", "urgent", "critical", "crash", "failed", "surge", "record", "beats", "wins", "launch", "dropped")):
        score += 0.12
    return round(clamp(score), 3)


def infer_novelty(text: str) -> float:
    lower = text.lower()
    score = 0.25
    if any(w in lower for w in ("new", "launch", "launched", "release", "released", "dropped", "now", "first", "just", "here")):
        score += 0.35
    if re.search(r"\b\d+(?:\.\d+)+\b", text):
        score += 0.18
    if any(w in lower for w in ("record", "beats", "ahead", "#1", "1st", "first")):
        score += 0.12
    return round(clamp(score), 3)


def make_claim(actor: str, action: str, obj: str, entities: Sequence[str]) -> Dict[str, str]:
    subject = entities[0] if entities else actor
    predicate = action.replace("_", " ") or "emit"
    claim_obj = obj if obj and obj != subject else "event"
    return {"subject": subject, "predicate": predicate, "object": clean_text(claim_obj, 180)}


def row_to_engine_event(row: Dict[str, Any], index: int) -> Dict[str, Any]:
    """Return booth-native, engine-ready event objects.

    The JSON output is intentionally NOT a `raw` bypass. It is the structured
    event shape the deterministic booth already understands, plus extra metadata
    the current HTML can ignore safely and future HTML can use for threading,
    filtering, inquiry, and richer narration.
    """
    raw = row.get("raw") if isinstance(row.get("raw"), dict) else {}

    actor = first_nonempty(tag_value_from_row(row, "actor"), row.get("actor"), raw.get("actor"), raw.get("source"), raw.get("name"), "tape")
    action = first_nonempty(tag_value_from_row(row, "action"), row.get("action"), raw.get("action"), raw.get("event"), raw.get("type"), "emit")
    obj = first_nonempty(tag_value_from_row(row, "object"), row.get("object"), raw.get("object"), raw.get("title"), raw.get("message"), raw.get("body"), raw.get("text"), raw.get("value"))
    valence = first_nonempty(tag_value_from_row(row, "valence"), row.get("valence"), raw.get("valence"))
    time_value = first_nonempty(tag_value_from_row(row, "time"), row.get("time"), raw.get("published"), raw.get("time"), raw.get("timestamp"), raw.get("created_at"))
    source_value = first_nonempty(tag_value_from_row(row, "source"), row.get("source"), raw.get("link"), raw.get("source"))
    feed = first_nonempty(tag_value_from_row(row, "feed"), raw.get("loom_feed_label"), raw.get("feed"), urllib.parse.urlparse(source_value).netloc)
    feed_type = first_nonempty(tag_value_from_row(row, "feed_type"), raw.get("loom_feed_type"), row.get("kind"), "event")

    actor = clean_text(actor, 180) or "tape"
    action = slug_action(action)
    obj = clean_text(obj, 240)
    if not valence:
        valence = infer_valence(" ".join([actor, action, obj]))
    valence = clean_text(valence, 40) or "calm"

    priority = clamp(float(row.get("priority", raw.get("priority", 0.5)) or 0.5))
    text = " ".join([actor, action.replace("_", " "), obj, source_value])
    topic = classify_topic(text, source_value)
    entities = extract_entities(" ".join([obj, actor]))
    intensity = infer_intensity(text, valence, priority)
    novelty = infer_novelty(text)
    claim = make_claim(actor, action, obj, entities)
    primary_entity = entities[0] if entities else actor
    thread = f"{topic}:{safe_tag_value(primary_entity).lower()}"

    tags = [
        f"actor:{safe_tag_value(actor)}",
        f"action:{safe_tag_value(action)}",
        f"valence:{safe_tag_value(valence)}",
        f"topic:{safe_tag_value(topic)}",
        f"kind:{safe_tag_value(feed_type)}",
    ]
    if obj:
        tags.append(f"object:{safe_tag_value(obj)}")
    if time_value:
        tags.append(f"time:{safe_tag_value(time_value)}")
    if source_value:
        tags.append(f"source:{safe_tag_value(source_value)}")
    if feed:
        tags.append(f"feed:{safe_tag_value(feed)}")
    for ent in entities[:5]:
        tags.append(f"entity:{safe_tag_value(ent)}")
    tags.append(f"thread:{safe_tag_value(thread)}")

    # Preserve existing stack/feed tags, but do not duplicate.
    for key in ("stack", "feed", "feed_type"):
        value = tag_value_from_row(row, key) or str(raw.get("loom_" + key) or "")
        tag = f"{key}:{safe_tag_value(value)}" if value else ""
    ...
    return {
        ...
        "object": obj,
        "valence": valence,
        "lap": int(row.get("lap") or index + 1),
...
def write_engine_json_list(path: Path, rows: Sequence[Dict[str, Any]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(path.suffix + ".tmp")
    events = [row_to_engine_event(row, i) for i, row in enumerate(rows)]
    with tmp.open("w", encoding="utf-8") as f:
        json.dump(events, f, indent=2, ensure_ascii=False)
    os.replace(tmp, path)


# Backwards name used by the existing recorder flow. It now writes engine-ready JSON.
def write_booth_json_list(path: Path, rows: Sequence[Dict[str, Any]]) -> None:
    write_engine_json_list(path, rows)
...
def fetch_url(url: str, timeout: int = 20) -> bytes:
    req = urllib.request.Request(
        url,
        headers={
            # A browser-like UA avoids unnecessary feed blocking on sites such as Reddit.
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0 Safari/537.36 LoomTapeRecorder/1.0",
...
class RssSource(Source):
    ...
    def poll(self) -> List[Dict[str, Any]]:
        ...
        def text_from(node: ET.Element, *names: str) -> str:
            for name in names:
        ...
        # RSS items
        rss_items = list(root.findall(".//item")) + list(root.findall(".//{*}item"))
        for item in rss_items:
            ...
            link = text_from(item, "link")
            pub = text_from(item, "pubDate", "date", "published", "updated")
            desc = text_from(item, "description")
...
def record(cfg: RecorderConfig) -> Tuple[int, int]:
    ...
    if cfg.write_json:
        write_booth_json_list(cfg.out_json, all_rows)
