...
let loomImage=null, loomImageUrl="";
...
const SOURCE_SYMBOLS="abcdefghijklmnopqrstuvwxyz .,!?;:";
const LOOM_PIXEL_CODEC={magic:[76,80,88,49],pad:14,cell:12,membrane:1};
...
function readVarint(bytes,start){
 ...
 while(pos<bytes.length){
  const b=bytes[pos++];
  result |= (b & 0x7f) << shift;
  if(!(b & 0x80)) return {value:result>>>0,next:pos};
  shift += 7;
  if(shift>28) break;
 }
 return null;
}
function decodePayloadBytesToText(payloadBytes,expectedChars){
 const ids=[];
 let pos=0;
 while(pos<payloadBytes.length && ids.length<expectedChars){
  const parsed=readVarint(payloadBytes,pos);
  if(!parsed) break;
  pos=parsed.next;
  const id=(parsed.value>>>0)-1;
  if(id<0 || id>=SOURCE_SYMBOLS.length) break;
  ids.push(id);
 }
 return {
  ids:ids,
  text:ids.map(id=>SOURCE_SYMBOLS[id]||" ").join(""),
  consumed:pos
 };
}
function drawLoomPreview(){
 const canvas=$('loomPreview'),ctx=canvas.getContext('2d');
 const W=canvas.width=canvas.clientWidth*devicePixelRatio;
 const H=canvas.height=Math.max(120,Math.round(canvas.clientWidth*0.56))*devicePixelRatio;
 ctx.clearRect(0,0,W,H);
 ctx.fillStyle='#020309';
 ctx.fillRect(0,0,W,H);
 if(!loomImage){
  ctx.fillStyle='#99a4b3';
  ctx.fillText('no loom-pixel image',20,30);
  return;
 }
 const scale=Math.min(W/loomImage.width,H/loomImage.height);
 const drawW=loomImage.width*scale, drawH=loomImage.height*scale;
 const x=(W-drawW)/2, y=(H-drawH)/2;
 ctx.drawImage(loomImage,x,y,drawW,drawH);
}
async function loadLoomFile(file){
 if(loomImageUrl) URL.revokeObjectURL(loomImageUrl);
 loomImageUrl=URL.createObjectURL(file);
 const img=new Image();
 await new Promise(function(resolve,reject){
  img.onload=resolve;
  img.onerror=reject;
  img.src=loomImageUrl;
 });
 loomImage=img;
 $('loomStatus').textContent=`loaded ${file.name} · ${img.width}x${img.height}`;
 drawLoomPreview();
}
function decodeLoomPixel(){
 if(!loomImage) return alert('Load a loom-pixel PNG first.');
 const cols=parsePositiveInt($('loomCols').value,32);
 const cell=parsePositiveInt($('loomCell').value,LOOM_PIXEL_CODEC.cell);
 const pad=parsePositiveInt($('loomPad').value,LOOM_PIXEL_CODEC.pad);
 const membrane=parsePositiveInt($('loomMembrane').value,LOOM_PIXEL_CODEC.membrane);
 const usableWidth=loomImage.width - pad*2;
 const usableHeight=loomImage.height - pad*2;
 const rows=Math.floor(usableHeight / Math.max(1,cell));
 if(cols<1 || rows<1 || usableWidth<cell){
  $('loomDecoded').textContent='geometry invalid for loom-pixel decode';
  $('loomReport').textContent=`image: ${loomImage.width}x${loomImage.height}\ncols=${cols} cell=${cell} pad=${pad} membrane=${membrane}\nstatus: invalid geometry`;
  return;
 }
 const scratch=document.createElement('canvas');
 scratch.width=loomImage.width;
 scratch.height=loomImage.height;
 const sctx=scratch.getContext('2d');
 sctx.drawImage(loomImage,0,0);
 const pixels=sctx.getImageData(0,0,scratch.width,scratch.height).data;
 const framedCount=rows*cols;
 const bytes=[];
 const inner=cell - membrane*2;
 const half=inner/2;
 for(let i=0;i<framedCount;i++){
  const x=pad + (i % cols) * cell;
  const y=pad + Math.floor(i / cols) * cell;
  let byte=0;
  const base=loomPixelBaseColor(i,framedCount);
  for(let q=0;q<4;q++){
   const sx=x + membrane + (q % 2) * half + half/2;
   const sy=y + membrane + Math.floor(q / 2) * half + half/2;
   const sample=sampleRgb(pixels,scratch.width,scratch.height,sx,sy);
   let bestValue=0,bestDist=Infinity;
   for(let v=0;v<4;v++){
    const target=loomPixelVariant(base,v);
    const dist=rgbDistance(sample,target);
    if(dist<bestDist){
      bestDist=dist;
      bestValue=v;
    }
   }
   byte=(byte<<2) | bestValue;
  }
  bytes.push(byte);
 }
 const magic=String.fromCharCode(bytes[0]||0,bytes[1]||0,bytes[2]||0,bytes[3]||0);
 if(magic!=='LPX1'){
  $('loomDecoded').textContent='frame error: expected LPX1 magic';
  $('loomReport').textContent=`magic: ${magic || '(none)'}\nstatus: wrong frame or wrong geometry\nimage: ${loomImage.width}x${loomImage.height}\nrows=${rows} cols=${cols}`;
  return;
 }
 let pos=4;
 const byteLen=readVarint(bytes,pos);
 if(!byteLen){ $('loomDecoded').textContent='frame error: byte length varint unreadable'; return; }
 pos=byteLen.next;
 const charLen=readVarint(bytes,pos);
 if(!charLen){ $('loomDecoded').textContent='frame error: char length varint unreadable'; return; }
 pos=charLen.next;
 if(pos+4>bytes.length){
  $('loomDecoded').textContent='frame error: checksum bytes missing';
  return;
 }
 const checksum=[bytes[pos],bytes[pos+1],bytes[pos+2],bytes[pos+3]].map(function(b){ return (b||0).toString(16).padStart(2,'0'); }).join('');
 pos += 4;
 const payloadBytes=bytes.slice(pos,pos + byteLen.value);
 const decoded=decodePayloadBytesToText(payloadBytes,charLen.value);
 const computed=fnv1a(decoded.text);
 const checksumOk=computed===checksum;
 $('loomDecoded').textContent=(decoded.text || '(empty)') + `\n\nraw bytes:\n${payloadBytes.slice(0,128).map(function(b){ return b.toString(16).padStart(2,'0'); }).join(' ')}`;
 $('loomReport').textContent=
  `frame magic: ${magic}` +
  `\nframed cells read: ${bytes.length}` +
  `\npayload bytes declared: ${byteLen.value}` +
  `\npayload bytes recovered: ${payloadBytes.length}` +
  `\nchars declared: ${charLen.value}` +
  `\nchars recovered: ${decoded.ids.length}` +
  `\nchecksum declared: ${checksum}` +
  `\nchecksum computed: ${computed}` +
  `\nchecksum ok: ${checksumOk}` +
  `\nimage: ${loomImage.width}x${loomImage.height}` +
  `\nrows=${rows} cols=${cols} cell=${cell} pad=${pad} membrane=${membrane}` +
  `\n\nThis decoder samples fixed loom-pixel quadrants. If the image was resized, filtered, or encoded with different geometry, failure is expected and honest.`;
}
...
$('loomFile').onchange=e=>{const f=e.target.files&&e.target.files[0]; if(f)loadLoomFile(f)};
...
$('decodeLoom').onclick=decodeLoomPixel;
