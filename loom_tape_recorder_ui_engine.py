...
- .tape.json    booth-compatible JSON event list ({actor, action, object, valence, lap, priority})
...
def row_to_booth_event(row: Dict[str, Any], index: int) -> Dict[str, Any]:
    # The booth engine's canonical tape shape is NOT {tags:[...]}.
    # It expects plain event objects like the built-in TAPES:
    # {actor, action, object, valence, lap, priority}.
    ...
    valence = tag_value_from_row(row, "valence") or str(raw.get("valence") or "calm")
    ...
    feed = tag_value_from_row(row, "feed") or str(raw.get("loom_feed_label") or "")
    feed_type = tag_value_from_row(row, "feed_type") or str(raw.get("loom_feed_type") or "")

    event: Dict[str, Any] = {
        "actor": clean_text(actor, 180) or "tape",
        "action": slug_action(action),
        "object": clean_text(obj, 240),
        "valence": clean_text(valence, 40) or "calm",
        "lap": index + 1,
        "priority": max(0.0, min(1.0, float(row.get("priority", 0.5) or 0.5))),
    }

    # Extra fields are harmless to the booth, but useful for query/debug/future tape tools.
    if time_value:
        event["time"] = clean_text(time_value, 80)
    if source_value:
        event["source"] = clean_text(source_value, 240)
    if feed:
        event["feed"] = clean_text(feed, 80)
    if feed_type:
        event["feed_type"] = clean_text(feed_type, 40)
...
def write_booth_json_list(path: Path, rows: Sequence[Dict[str, Any]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(path.suffix + ".tmp")
    events = [row_to_booth_event(row, i) for i, row in enumerate(rows)]
    with tmp.open("w", encoding="utf-8") as f:
