...
def row_to_loader_row(row: Dict[str, Any], index: int) -> Dict[str, Any]:
    """Return the exact shape the current booth upload loader expects.

    Important booth detail: the loader normalizes uploaded rows from `tags`, then
    sets `raw: row.raw || row`. Later `narrate()` returns `r.raw` directly when
    it exists. If raw is an object, playback becomes `[object Object]`.

    So the recorder JSON must keep the tag grammar AND make `raw` a string.
    NDJSON remains the full archival/debug object.
    """
    raw = row.get("raw") if isinstance(row.get("raw"), dict) else {}

    actor = tag_value_from_row(row, "actor") or str(raw.get("actor") or raw.get("source") or raw.get("name") or "tape")
    action = tag_value_from_row(row, "action") or str(raw.get("action") or raw.get("event") or raw.get("type") or "emit")
    obj = tag_value_from_row(row, "object") or str(raw.get("object") or raw.get("title") or raw.get("message") or raw.get("text") or "")
    valence = tag_value_from_row(row, "valence") or str(raw.get("valence") or infer_valence(" ".join([actor, action, obj])))
    time_value = tag_value_from_row(row, "time") or str(raw.get("published") or raw.get("time") or raw.get("timestamp") or "")
    source_value = tag_value_from_row(row, "source") or str(raw.get("link") or raw.get("source") or "")
    ...
    # Human line for narrate() fallback. Use present-ish source verbs rather than
    # pre-rendering grammar; the booth can still dress/query via actor/action/object.
    raw_line = " ".join(part for part in [actor, action.replace("_", " "), obj] if part).strip()
    ...
    # Preserve stack/feed tags if present.
    for key in ("stack", "feed", "feed_type"):
        value = tag_value_from_row(row, key) or str(raw.get("loom_" + key) or "")
        if value:
            tags.append(f"{key}:{safe_tag_value(value)}")

    return {
        "tags": tags,
        "priority": max(0.0, min(1.0, float(row.get("priority", 0.5) or 0.5))),
        "raw": raw_line,
    }
...
def write_booth_json_list(path: Path, rows: Sequence[Dict[str, Any]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(path.suffix + ".tmp")
    events = [row_to_loader_row(row, i) for i, row in enumerate(rows)]
    with tmp.open("w", encoding="utf-8") as f:
